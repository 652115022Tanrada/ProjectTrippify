...
<template>
  <div class="min-h-screen flex flex-col bg-[#FFFFFF] text-gray-800 font-kanit">
      ...
      <div class="w-full max-w-6xl space-y-8">
        ...
        >
          <div
            ...
            :key="trip.tripId"
            c class="p-5 sm:p-6 rounded-2xl bg-[#EEEDED] shadow-xl hover:shadow-2xl hover:bg-sky-50 transition-all duration-300 transform hover:scale-105 cursor-pointer"
            @click="viewTripDetail(trip.tripId)"
          >
            <div class="flex items-center space-x-3 sm:space-x-4">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                class="h-8 w-8 sm:h-10 sm:w-10 text-[#0D1282]"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                stroke-width="2"
        ...
        <div v-if="savedTrips.length > 0" class="grid grid-cols-2 gap-6 w-full">
          <div v-for="trip in savedTrips" :key="trip.tripId"
            ...
            <div class="mt-3 sm:mt-4 border-t pt-3 sm:pt-4 border-gray-100">
                            ...
                            Total Cost: <span class="text-green-600">{{ trip.total_trip_cost }}
        ...
        <div
          v-else
          class="text-center py-16 sm:py-20 text-gray-500 text-lg sm:text-xl font-semibold"
        >
    ...
    >
      <div
        class="bg-white p-6 sm:p-8 rounded-2xl shadow-2xl space-y-6 w-full max-w-sm relative transform scale-100 transition-transform duration-300"
      >
        <button
          @click="joinTripLinkModal = false"
          class="absolute top-4 right-4 text-gray-500 hover:text-gray-800 text-xl sm:text-2xl font-bold transition"
        >
          ✕
        </button>
        <h2 class="text-xl sm:text-2xl font-bold text-sky-700 text-center">
          Join Trip via Link
        </h2>
        <input
          ...
          class="w-full border border-gray-300 rounded-lg px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base focus:ring-4 focus:ring-sky-200 focus:border-sky-500 outline-none transition"
        />
        <button
          @click="submitJoinLink"
          class="w-full bg-[#0D1282] text-white px-4 py-2 sm:py-3 rounded-lg font-semibold hover:bg-sky-900 transition transform hover:scale-95 text-sm sm:text-base"
        >
          Join Trip
        </button>
      </div>
