...
const filteredLocations = computed(() => {
  if (!tripPlan.value || !tripPlan.value.days) return [];

  if (selectedDayIndex.value === null) {
    // รวมทุกวัน + ใส่ day ให้ location
    return tripPlan.value.days.flatMap((day, index) => {
      return (day.locations || []).map((loc) => ({
        ...loc,
        day: index + 1, // เพิ่ม day property
      }));
    });
  }
  ...
  return (day.locations || []).map((loc) => ({
    ...loc,
    day: selectedDayIndex.value + 1, // เพิ่ม day property
  }));
});
...
<template>
  <div class="flex bg-[#FFFFFF]">
    ...
    <main
      class="flex-1 flex flex-col md:flex-row overflow-hidden md:ml-48 pt-10"
    >
      <div v-if="tripPlan" class="flex flex-col md:flex-row w-full">
        <div class="w-full md:w-[50%] p-8 md:p-8 overflow-y-auto">
          <div class="bg-[#EEEDED] rounded-3xl shadow-lg p-8">
            ...
            >
              <Tripmap
                v-model:locations="filteredLocations"
                :trip-plan="tripPlan"
              />
            </div>
            <div
            ...
            <div v-show="activeTab === 'plan'">
              ...
              >
                ...
                <div
                  class="w-16 h-16 rounded-full bg-white/20 flex items-center justify-center z-10"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
