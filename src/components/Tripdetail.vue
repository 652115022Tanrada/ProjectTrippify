...
const selectedDayIndex = ref(null); // วันที่กำลังดู

// ✅ computed เพื่อเลือก location ตามวันที่เลือก
const filteredLocations = ref([]);

watch(
  [tripPlan, selectedDayIndex],
  ([newPlan, newDay]) => {
    if (!newPlan?.days) return;
    let newLocs = [];

    if (newDay === null) {
      newLocs = newPlan.days.flatMap((day, index) =>
        (day.locations || []).map((loc) => ({
          ...loc,
          day: index + 1,
        }))
      ...
      const day = newPlan.days[newDay];
      newLocs = (day.locations || []).map((loc) => ({
        ...loc,
        day: newDay + 1,
      }));
    }

    // ✅ compare ก่อนอัปเดตจริง
    const old = JSON.stringify(filteredLocations.value);
    const now = JSON.stringify(newLocs);
    if (old !== now) filteredLocations.value = newLocs;
  },
...
const handleExpenseClick = () => {
  // ตรวจสอบว่ามี tripId แล้วหรือยัง
  if (!tripPlan.value?.tripId) {
    Swal.fire({
      icon: "warning",
      title: "Trip not saved yet!",
      text: "Please save your trip before viewing expenses.",
      confirmButtonColor: "#0ea5e9",
    });
    return;
  }
  goToPage(`/expense/${tripPlan.value.tripId}`);
};
    ...
    const { data } = await axios.get(
      `${API_URL}/api/places/search`,
      {
        params: {
          ...
          to_location:
            trip.value?.to_location || tripPlan.value?.tripName || "Thailand",
        },
        withCredentials: true,
      }
    );
      ...
      <div v-if="tripPlan" class="flex flex-col md:flex-row w-full">
        <div class="w-full md:w-[50%] p-8 md:p-8 overflow-y-auto">
          <div class="bg-[#EEEDED] rounded-3xl shadow-lg p-8">
            ...
            <div
              class="w-full h-64 md:fixed md:top-10 md:right-10 md:w-[600px] md:h-[90vh] rounded-xl shadow-lg overflow-hidden">
              <Tripmap v-model:locations="filteredLocations" :trip-plan="tripPlan" />
            </div>
            <div class="flex flex-col md:flex-row items-stretch md:items-center justify-between mb-6 gap-4 md:gap-0">
              ...
              <button @click="handleExpenseClick"
            ...
            <div v-show="activeTab === 'plan'">
              <div class="mb-6 flex justify-end gap-2 md:gap-4">
                <!-- Leader buttons -->
                <template v-if="!trip?.tripId || trip?.role === 'leader'">
                  <!-- Cancel Trip (เทา) -->
                ...
                <div class="relative p-2">
                  ...
                  <button @click="showDays = !showDays"
                    class="px-4 py-2 rounded-full bg-[#0D1282] text-[#EEEDED] flex items-center gap-2 hover:bg-[#F0DE36] hover:text-[#0D1282] font-medium text-sm transition-all">
                    <span>
                      {{
                  ...
                  <transition name="fade">
                    <div v-show="showDays"
                      class="absolute mt-2 w-40 bg-[#FFFFFF] text-[#000000] rounded-md shadow-lg overflow-hidden z-50">
                      <button @click="
                        selectedDayIndex = null;
                      showDays = false;
                      " :class="[
                          'w-full text-left px-4 py-2 hover:bg-[#777777] hover:text-[#FFFFFF] transition-all',
                          selectedDayIndex === null
                            ? 'bg-[#777777] text-[#FFFFFF]'
                            : '',
                        ]">
                        All
                      </button>
                      <button v-for="(day, index) in tripPlan.days" :key="index" @click="
              ...
              <div class="mb-8 bg-white border-b border-gray-200 p-6 md:p-8 rounded-md shadow-sm w-full max-w-full">
                ...
                <!-- ✅ ตาราง: แสดงเฉพาะหน้าจอขนาด md ขึ้นไป -->
                <div class="w-full overflow-x-hidden hidden md:block">
                ...
                <!-- ✅ แสดงเฉพาะบนมือถือ -->
                <div class="block md:hidden mt-2 sm:mt-4 text-gray-700 text-xs sm:text-sm md:text-base">
                  <p>
                ...
                <!-- ✅ ส่วนนี้ยังคงแสดงทั้งสองขนาดได้ (optional ถ้าต้องการให้แสดงทุกขนาด) -->
                <p class="hidden md:block mt-2 sm:mt-4 text-gray-700 text-sm md:text-base">
                  How to get there:
                  {{ transportInfo?.how_to_get_there || "N/A" }}
                </p>
              </div>
              ...
              <div v-for="(day, index) in tripPlan.days" :key="day.id || index"
                v-show="selectedDayIndex === null || selectedDayIndex === index"
                class="mb-8 bg-white/95 p-6 rounded-2xl shadow-lg border border-[#EEEDED]">
                <!-- Header -->
                <h2 class="text-xl md:text-2xl font-bold text-[#0D1282] mb-4 flex items-center">
                  <span
                ...
                <div class="md:hidden mt-4 space-y-4">
                  <draggable v-model="day.locations" :group="'locations'" item-key="name" :disabled="!showEditControls">
                    <template #item="{ element: loc, index: i }">
                      <div class="bg-white p-4 rounded-xl shadow border hover:bg-gray-50 transition cursor-grab">
